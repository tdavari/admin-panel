...
Route::post('/app/email/send', $controller_path . '\apps\Email@sendEmail')->name('app-email-send');
...
#######################test###
Route::get('/send-email', [Email::class, 'sendEmail'])->name('send.email');

Route::middleware(['auth:sanctum', 'verified'])->group(function () use ($controller_path) {
  // Routes within this group will only be accessible to authenticated users

  Route::get('/test', function () {
    return view('layouts.layoutMaster');
  });

  Route::get('/ixp/table', $controller_path . '\ixp\IxpController@table')->name('ixp.table');
  Route::get('/ixp/table/updateall', $controller_path . '\ixp\IxpController@updateall')->name('ixp.table.updateall');

  // ... (Add other protected routes here)
  Route::get('/netbox', $controller_path . '\netbox\NetboxController@netboxUpdate')->name('netbox.update');
  Route::get('/netboxstream', $controller_path . '\netbox\NetboxController@netboxStream')->name('netbox.stream');

  Route::get('/set-key-value', $controller_path . '\netbox\NetboxController@setKeyValue');

  // ixp request vlan
  Route::get('/ixp/vlan-request', $controller_path . '\ixp\VlanController@index')->name('ixp.vlan-request');
});
